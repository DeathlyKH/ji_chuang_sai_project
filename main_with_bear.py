...
def init_camera():
    ...
    if cap.isOpened():
        ...
        if ret and test is not None and test.size > 0:
            print(f"[OK] 摄像头打开成功: {test.shape[1]}x{test.shape[0]}")
            # 设置分辨率
            if IS_ARM:
    ...
    if IS_WINDOWS:
        ...
        if cap.isOpened():
            ret, test = cap.read()
            if ret and test is not None and test.size > 0:
                print(f"[OK] 摄像头(DSHOW)打开成功: {test.shape[1]}x{test.shape[0]}")
                return cap
            cap.release()
    ...
    if cap.isOpened():
        ret, test = cap.read()
        if ret and test is not None and test.size > 0:
            print(f"[OK] 摄像头(V4L2)打开成功: {test.shape[1]}x{test.shape[0]}")
            return cap
        cap.release()
    
    print("[ERR] 所有方法都无法打开摄像头！")
    print("[提示] 请检查：1)摄像头是否插好 2)是否被其他程序占用 3)驱动是否正常")
    ...
    class DummyCapture:
        def __init__(self):
            ...
            cv2.putText(self.frame, "Check USB connection", (20, 140),
...
def main():
    ...
    def _init_speech():
        ...
        try:
            # ARM平台自动跳过语音（Whisper+PyTorch太重）
            if IS_ARM:
                speech_result[0] = "ERROR:ARM平台自动禁用语音"
                print("[INFO] ARM平台，语音模块自动禁用（节省内存）")
                return
